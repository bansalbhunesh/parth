# Pramaan â€” Submission Checklist (ET AI Hackathon 2.0, Phase 2)

> **Pre-flight the day you submit?** Use [`FINAL_SUBMISSION_CHECKLIST.md`](FINAL_SUBMISSION_CHECKLIST.md)
> â€” every judge-facing URL, artifact path, and truth-gate in one place. This file
> tracks the deliverables; that file verifies them.
>
> One row per deliverable: what the form wants â†’ the exact artifact â†’ status.
...
- [x] **Phase 2 submission deadline (live Unstop record):** `2026-07-22, 23:59:21 IST` (re-confirm on the dashboard before submitting â€” organizers may adjust)
- [ ] Submit by **21 July evening** (leave a full night of slack for cold starts, upload retries, form surprises).
- [ ] Freeze window: **no risky changes after 19 July** â€” final 3 days (20â€“22 July) are verify/rehearse/record only.

## Deliverables

| # | Deliverable (per rules) | Artifact | Status |
|---|---|---|---|
| 1 | **Public GitHub repository** | `https://github.com/bansalbhunesh/parth` | âœ… public since 2026-07-03; CI badge verified "passing" logged-out. NOTE: the repo home renders `main`'s README â€” keep `main` fast-forwarded to the submission tip |
| 2 | **Pitch video** (2:50) | YouTube unlisted link â†’ also placed in README "Judges: start here" Â§3, `UNSTOP_SUBMISSION.md`, `FINAL_SUBMISSION_CHECKLIST.md`, and `docs/detailed_submission.html` | â¬œ **BLOCKER** â€” record per `docs/VIDEO_RUNBOOK.md` (script: `PITCH.md`) |
| 3 | **Architecture document** | [`docs/ARCHITECTURE.md`](ARCHITECTURE.md) (+ `docs/pipeline-diagram.svg`) | âœ… |
| 4 | **Impact model** (quantified business benefit) | [`docs/BUSINESS.md`](BUSINESS.md) â€” every figure cited | âœ… |
| 5 | Pitch deck (if the form takes a file) | [`docs/Pramaan_Deck.pdf`](Pramaan_Deck.pdf) â€” 12 pages, regenerate with `python scripts/export_deck.py` after any deck edit | âœ… |
| 6 | Live demo URL | `https://parth-tan.vercel.app/judge` (front) Â· `https://parth-1-ma30.onrender.com/health` (API) | âœ… live & verified 2026-07-16 against deployed `main` (`6adc9ad`): `verify_live.py` passed 10/10 in 142 s Â· `/health` ready Â· all three PS4 layers live Â· deep probe 5 findings/13.9 s Â· `/analyze` and `/analyze/stream` both `mode: llm` with 5 findings Â· frontend `/` and `/judge` 200. `/ocr-check` remains the hosted Docker/Tesseract readiness gate. Re-verify after the final submission commit and again on submission day with `make verify-live` |
| 7 | Team details | 1â€“4 members, one team per person â€” from the Unstop registration | â¬œ confirm roster matches registration |

## Pre-submission gates (run in this order, same day)

1. `main` fast-forwarded to the branch tip (Render + Vercel deploy from `main`).
2. Fresh `GEMINI_API_KEY` set in Render â†’ Manual Deploy â†’ latest commit.
3. `make verify-live` â†’ **`GREEN -- demo away.`** (checks deployed commit, PS4
...
5. Repo public â†’ open the README logged-out: CI badge green, screenshots load,
...
- Hit the live app once (~40 s Render cold start), run one warm-up Analyze,
  then `make verify-live` again. Pre-flight ritual: `PITCH.md` Â§Pre-flight.

## Do NOT touch after the final gate

`eval/` + committed results Â· `data/samples/real/` + `PROVENANCE.md` Â·
`data/corpus/` (git add -f trap) Â· `gemini-2.5-flash` pin Â· git history.
# Pramaan â€” Final Submission Checklist (Phase 11)

> The single pre-flight page for the ET AI Hackathon 2026 Â· PS4 submission. It
...
| Judge Mode | https://parth-tan.vercel.app/judge | Load deviation demo â˜… â†’ Analyze streams; provenance chip visible |
...
| Pitch script | [`../PITCH.md`](../PITCH.md) | 3â€“4 min; carries the safe line verbatim (Â§3) |
...
| Pitch video | â¬œ **BLOCKER** â€” YouTube (Unlisted) â†’ paste into README Â§3, `SUBMISSION_CHECKLIST.md` row 2, Unstop, and detailed submission source | record per `VIDEO_RUNBOOK.md` |

## 3. Repo / deploy state (fill on submission day)

- Latest commit on submission tip: `__________` (`git log -1 --format='%h %s'`)
- Branch merged to `main` and fast-forwarded (Render + Vercel deploy from `main`): â¬œ
- Render backend Manual-Deployed with fresh `GEMINI_API_KEY`: â¬œ
- `make verify-live` â†’ **`GREEN -- demo away.`**: â¬œ

### Judging-day environment (set in the Render dashboard, revert after)

- â¬œ **Raise the per-IP rate limits** â€” a venue's shared Wi-Fi NAT puts every
  ...
  defaults afterwards.
- â¬œ **Gateway balance** â€” the featured-model leg spends real credit
...
- â¬œ **Morning-of probe** â€” Gemini free-tier quota resets daily and has
  failed probes before (2026-07-14); run `make verify-live` the morning of,
  not just the night before.

## 4. Truth gates (all must pass â€” commands are copy-paste)

Run from repo root.

- [ ] **Tests** â€” `python -m pytest tests/ -q` â†’ 893 passed.
- [ ] **Lint** â€” `python -m ruff check .` â†’ clean.
- [ ] **Mandatory placeholders** â€” `python scripts/check_submission_ready.py` â†’ green only after the pitch-video URL is public and logged-out accessible.
- [ ] **Benchmark integrity untouched** â€” `git status --porcelain benchmarks/` â†’ empty (labels/scores/reviewer files pristine).
- [ ] **Frontend** â€” `cd frontend && npx tsc --noEmit && npm run build` â†’ clean.
- [ ] **Banned wording (automated)** â€” `python -m pytest tests/test_claims_register.py -q` â†’ green. The gate scans README, PITCH, COMPETITIVE, deck + detailed sources, all docs, frontend text, benchmark + sample docs for every `CLAIMS_REGISTER.md` banned phrase (affirmative use fails; same-sentence disclaimers pass).
- [ ] **No secrets** â€” no real API keys committed; `.env` gitignored; grep for `GEMINI_API_KEY=`/`sk-`/`AIza` returns only placeholders.
- [ ] **PDF sanity** â€” both PDFs open, page counts as above, each < 50 MB; detailed PDF text-extracts (searchable).
- [ ] **Number consistency** â€” benchmark reads **53 pairs Â· 129 labels Â· 17 systems Â· 64 clean negatives Â· recall 0.862 Â· precision 0.953 Â· F1 0.905 Â· FAR 0.000 Â· rule baseline 0.111 Â· p50 ~2.5 s** across /evidence, README, deck, pitch, detailed PDF, architecture docs.
- [ ] **Reviewer-2 honesty** â€” every benchmark surface says reviewer-2 **pending** and calls the automated cross-check **machine QA, not human**.
- [ ] **OCR** â€” `/ocr-check` ready on the hosted backend (tesseract in the Docker image).
- [ ] **Failover** â€” `/llm-check?deep=1` reports the true provider; failover is framed as **availability, not accuracy**.
- [ ] **Rule floor** â€” with no model, endpoints still return 200 and the UI labels a rule-floor result **inconclusive / low-recall**, never a clean bill of health.
...
> Pramaan is a prototype / hackathon build, **demo-hardened** â€” not production-grade.
> Keep claims accordingly (`docs/CLAIMS_REGISTER.md`).

## 1. Backend (Render â€” Docker runtime)
...
until a key exists â€” see `LLM_FAILOVER_RUNBOOK.md`).

## 2. Frontend (Vercel)

- `NEXT_PUBLIC_API=https://<backend-host>` (inlined at build â€” redeploy after a
...
key). Upload rejection spot-check per the security runbook Â§4.
...
- **Backend:** Render â†’ Deploys â†’ **Rollback** to the previous image, or
  ...
  env-disabled â€” to bypass it, roll back the image.
- **Frontend:** Vercel â†’ Deployments â†’ promote the previous build.
...
honestly reported as unavailable â€” never a silent zero and never a false claim.
...
> the single source of truth â€” the UI badge and docs defer to it.
...
| Docker (`Dockerfile.backend`) | âœ… yes | `apt-get install tesseract-ocr` in the image |
| docker-compose (`make docker`) | âœ… yes | backend service builds `Dockerfile.backend` |
| Render (`render.yaml`) | âœ… yes | `runtime: docker` â†’ builds `Dockerfile.backend` |
| Plain Python buildpack | âŒ no | buildpack can't `apt-get` â†’ graceful "OCR unavailable" |
...
`make verify-ocr` **skips cleanly** (exit 0) when no binary is installed â€” pass
...
1. Merge the branch so `render.yaml` (Docker runtime â†’ `Dockerfile.backend`) is on `main`.
2. Render â†’ the `pramaan-backend` service â†’ **Manual Deploy â†’ latest commit**.
   (First Docker build is slower than the buildpack â€” it apt-installs Tesseract.)
3. Confirm the live backend:
   ...
   ```
4. In Judge Mode, the upload panel shows **"OCR ready â€” scanned PDFs & images
   supported (Tesseract 5.x)"**. Upload a scanned PDF; the result carries an
   amber **"Read via OCR â€” verify critical values"** caveat.
...
| `TESSERACT_CMD` | â€” | path to the binary if not on PATH |
| `TESSDATA_PREFIX` | â€” | language-data dir |

## If OCR becomes risky near a deadline â€” disable honestly
...
- Not lossless (e.g. `GXT5` can read as `GXTS`) â€” hence the verify-values warning.
- English-only, best-effort; no per-page scanned detection (whole-document
  ...
  and claims-register rows 20â€“21 in [`CLAIMS_REGISTER.md`](CLAIMS_REGISTER.md).
- Image OCR here (Tesseract â†’ text) is distinct from `/analyze/vision` (an LLM
  reads the picture) â€” different paths, different contracts.
