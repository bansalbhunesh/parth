...
| 22 | LLM provider failover | **self-healing chain → deterministic floor** | `backend/llm.py`, `tests/test_failover.py`, `tests/test_failover_phase4.py`, `docs/LLM_FAILOVER_RUNBOOK.md` | measured | "on quota/429/rate-limit/timeout/unparseable output the demo fails over across the configured providers (order set by PRAMAAN_LLM_PROVIDER_ORDER; hosted demo: gemini → gateway (aicredits, benchmark-featured model, spend-guarded) → Groq) down to the deterministic rule engine; only configured providers are tried; a paid leg carries an hourly call budget that fails over when exhausted; `/llm-check` shows the live chain, budget state, and last failover" | "failover improves accuracy", "more accurate with failover", "never goes down", "100% uptime", "self-healing accuracy" | README, docs, demo, video | **reliability/availability only, NOT accuracy** — every leg is scored the same; the rule floor is deliberately low-recall (see #11) and computes from the real documents, never seeded labels; the Qwen gateway must be a genuinely separate quota (not Google's endpoint) | verified |
...
| 24 | Persisted case workflow | **SQLite-backed, per-case secret isolation** | `backend/case_store.py`, `tests/test_cases.py`, `docs/DATA_HANDLING.md` | measured | "a case's findings, drafted RFIs, and audit log persist to disk, gated by a per-case bearer secret shown once at creation; a wrong or missing secret 404s identically to a nonexistent case" | "multi-tenant", "production database", "durable storage", "enterprise persistence", "user accounts" | README, docs, demo | **single-instance SQLite, not a production database** — no mounted persistent volume on the hosted deploy (a redeploy wipes it, same caveat `jobs.py`'s cache already carries); tenant isolation is per-case-secret, not real user identity (no accounts anywhere in this codebase); no delete-my-case-data endpoint yet | verified |
