import { getHealth, getMultiProjectEval } from "../../lib/api";
...
export default async function EvidencePage() {
  const [health, multiEval] = await Promise.all([getHealth(), getMultiProjectEval()]);
    ...
    <main id="main-content">
      ...
      <div className="evidence-layout shell">
        ...
        <div className="evidence-body">
          <section className="evidence-section" id="deployment" aria-labelledby="deployment-title">
            ...
            {chain.length > 0 ? (
              <div className="failover-chain" aria-label="Configured analysis failover chain">
                <span className="failover-label">Failover chain</span>
                <ol>
                  {chain.map((provider) => (
                    <li key={provider}>{provider}</li>
                  ))}
                  <li className="failover-floor">rule floor</li>
                </ol>
                <p className="failover-note">
                  Each leg is time-boxed; if every provider fails, the deterministic rule floor answers and the result says so.
                </p>
              </div>
            ) : null}
          </section>

          <section className="evidence-section" id="benchmark" aria-labelledby="benchmark-title">
            <div className="evidence-section-head">
          ...
          <section className="evidence-section" id="fleet" aria-labelledby="fleet-title">
            <div className="evidence-section-head">
              <p className="section-number">03 · Across projects</p>
              <div><p className="section-kicker">Multi-project aggregate</p><h2 id="fleet-title">The same engine on {multiEval?.aggregate.projects ?? "—"} modeled projects</h2></div>
            </div>
            {multiEval ? (
              <dl className="evidence-ledger">
                <div><dt>Aggregate F1</dt><dd>{multiEval.aggregate.aggregate_f1.toFixed(3)}</dd><dd className="ledger-note"><small>{multiEval.aggregate.total_deviations} deviations across the fleet</small></dd></div>
                <div><dt>Aggregate recall</dt><dd>{multiEval.aggregate.aggregate_recall.toFixed(3)}</dd><dd className="ledger-note"><small>precision {multiEval.aggregate.aggregate_precision.toFixed(3)}</small></dd></div>
                <div><dt>Cx mapping accuracy</dt><dd>{multiEval.aggregate.aggregate_cx_accuracy.toFixed(3)}</dd><dd className="ledger-note"><small>deviation → commissioning gate</small></dd></div>
                <div><dt>Lead time recovered</dt><dd>{multiEval.aggregate.total_lead_time_weeks} wk</dd><dd className="ledger-note"><small>max on one finding: {multiEval.aggregate.max_lead_time_weeks} wk</small></dd></div>
              </dl>
            ) : (
              <p className="evidence-callout">The aggregate endpoint did not answer, so no fleet numbers are shown.</p>
            )}
          </section>

          <section className="evidence-section" id="verification" aria-labelledby="verification-title">
            <div className="evidence-section-head">
              <p className="section-number">04 · Verification</p>
          ...
          <section className="evidence-section" id="limitations" aria-labelledby="limitations-title">
            <div className="evidence-section-head">
          ...
          <section className="evidence-section" id="links" aria-labelledby="links-title">
            <div className="evidence-section-head">
